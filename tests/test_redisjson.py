...
def test_json_setgetdelete(client):
    assert client.REDISJSON.jsonset('foo', Path.rootPath(), 'bar')
    assert client.REDISJSON.jsonget('foo') == "bar"
    assert client.REDISJSON.jsonget('baz') is None
    assert client.REDISJSON.jsondel('foo') == 1
    assert client.REDISJSON.jsondel('foo') == 0 # second delete
    assert client.REDISJSON.exists('foo') == 0

@pytest.mark.integrations
@pytest.mark.redisjson
def test_justaget(client):
    client.REDISJSON.jsonset('foo', Path.rootPath(), 'bar')
    assert client.REDISJSON.jsonget('foo') == "bar"
